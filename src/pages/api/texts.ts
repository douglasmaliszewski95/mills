...
export default async function handler(
  ...
  const data: ResultSearchCMS = await fetch(
    `${process.env.ORACLE_CMS}/content/published/api/v1.1/items?q=type+eq+"custom_text"+AND+fields.content_page+co+"${description}"&channelToken=${channelToken}&fields=all&limit=250`,
    { headers: auth }
  )
    .then((res) => res.json())
    .catch((err) => {
