...
export default async function handler(
  ...
  if (req.method === 'GET') {
    ...
    if (token.access_token) {
      const response = await fetch(`${process.env.OCC_URL_STORE}/search?Ntt=${productName}`, {
        headers: {
      ...
      if (response.resultsList && response.resultsList?.records.length > 0) {
        ...
        const search = await fetch(`${process.env.OCC_URL_STORE}/products?productIds=${productIds}`, {
          headers: {
            "Authorization": `Bearer ${token.access_token}`
          }
        }).then(res => res.json())
