...
export const getFiliais = async (text: String) => {
  const result: ResultSearchCMS = await fetch(
    `${process.env.NEXT_PUBLIC_API_GRAPHQL}/api/filiais?description=${text}`
  )
    .then((res) => res.json())
    .catch((err) => console.error("Erro fetch filiais: " + err));
